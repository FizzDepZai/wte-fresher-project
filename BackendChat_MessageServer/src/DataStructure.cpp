...
//    hashDB.open("dbFiles/" + name + "Disk.kch", HashDB::OWRITER | HashDB::OCREATE);
        hashDB.open("../../../dbFiles/" + name + "Disk.kch", HashDB::OWRITER | HashDB::OCREATE);
}
...
int DataStructure::saveToCachDB(const char* key, const size_t& ksize, const char* value, const size_t& vsize) {
    ...
    ts.append(this->DataName);
    Poco::FastMutex::ScopedLock lock(DataStructure::_mutex);
    if (strcmp(this->DataName.data(), "MessageId") == 0) {
        Logger & logger = ServerLog::getLoggerForActivity(ServerLog::DATA_MSGID);
        logger.information(ts);
    }
    if (strcmp(this->DataName.data(), "MessageItem") == 0) {
        Logger & logger = ServerLog::getLoggerForActivity(ServerLog::DATA_MSGITEM);
        logger.information(ts);
    }
    if (strcmp(this->DataName.data(), "MessageOffline") == 0) {
        Logger & logger = ServerLog::getLoggerForActivity(ServerLog::DATA_MSGOFFLINE);
        logger.information(ts);
    }
    //    if(std::strcmp(DataName.c_str(), "MessageId"))
    //    cout << "IN SAVETOCACHDB " << "ksize is: " << ksize << " size of value: " << vsize << " database name is: " << DataName << endl;
    //    string tmp(value, vsize);
    //    cout<<"-------------"<<tmp<<endl;
    bool saveResult = this->cachDB.set(key, ksize, value, vsize);
//    if(strcmp(this->DataName.data(), "MessageId") == 0)
//    cout << "....................................save result is: " << saveResult << endl;
